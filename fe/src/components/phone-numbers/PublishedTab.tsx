...
import { Phone, Users, Trash2, Edit2, Shield } from "lucide-react";
...
export function PublishedTab({ numbers, onEdit, onDelete }: PublishedTabProps) {
  if (numbers.length === 0) {
    return (
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-zinc-50 text-zinc-500 uppercase text-[10px] tracking-wider border-b border-zinc-200 font-bold">
            ...
            <th className="px-5 py-3 text-left">Purchased By</th>
          ...
          {numbers.map((num) => (
            <tr key={num.id} className="hover:bg-zinc-50/50 transition-colors">
              ...
              <td className="px-5 py-4 whitespace-nowrap text-center">
                <span className={`inline-block px-2.5 py-0.5 rounded-sm text-[10px] font-bold uppercase tracking-wider border ${
                  num.status === "active"
                    ? "bg-emerald-50 text-emerald-700 border-emerald-250"
                    : "bg-zinc-50 text-zinc-650 border-zinc-200"
                }`}>
                  {num.status === "active" ? "Sold" : "Available"}
                </span>
              </td>
