...
import { type HttpMethod, type PresetToolParameter, type ToolParameter, type KeyValueItem } from "@/components/http";
...
const TYPE_SAMPLE_VALUES: Record<string, string> = {
    string: "sample_text",
    number: "5",
    boolean: "true",
    array: "[]",
...
export function generateSampleValue(type: string): string {
                ...
                return (
                    <div key={parameter.name} className="space-y-1.5">
                        <div className="flex items-center gap-2">
                            ...
                            {parameter.required && <span className="text-xs text-destructive font-medium">required</span>}
                        </div>
...
export function HttpToolTestDialog({
    ...
    const openJsonEditDialog = (source: ParameterSource, parameterName: string) => {
        ...
        try {
            ...
            // Keep incomplete or invalid JSON as-is
        }
    ...
    const handleTestTool = async () => {
        try {
            ...
            const response = await ToolsService.test(toolUuid, {
            ...
            setTestError(
                formatApiErrorDetail(
                    caughtError?.response?.data?.message || caughtError?.message || "Failed to test tool"
                )
            );
        } finally {
            setIsTesting(false);
